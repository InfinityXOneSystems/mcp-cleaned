...
class MockLLMAdapter:
    ...
    async def call_model(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        keyword = kwargs.get('keyword') or 'unknown'
        # simple deterministic scoring by presence of urgent words
        text = kwargs.get('text','')
        urgency = 0.8 if any(w in text.lower() for w in ['urgent','emergency','behind','delinquent']) else 0.2
        emotional_stress = 0.7 if any(w in text.lower() for w in ['stressed','panic','desperate']) else 0.1
        confidence = 0.6
        parsed = {
            'urgency': urgency,
