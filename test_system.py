...
def test_database():
    ...
    try:
        ...
        log_test(
            "Paper Positions", positions_count >= 0, f"{positions_count} positions"
        )
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if r.status_code == 200:
                ...
                log_test(
                    ...
                    True,
                    f"Total value: ${data.get('total_value', 0):.2f}, Positions: {data.get('num_positions', 0)}",
                )
            else:
                log_test("Dashboard API - Portfolio", False, f"Status {r.status_code}")
    except Exception as e:
        log_test(
            "Dashboard API - Portfolio", False, f"Not running or error: {str(e)[:50]}"
        )
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if r.status_code == 200:
                ...
                log_test(
                    ...
                    True,
                    f"{len(data)} categories, {total_sources} total sources",
                )
            else:
                log_test(
                    "Intelligence API - Categories", False, f"Status {r.status_code}"
                )
    except Exception as e:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if r.status_code == 200:
                ...
                log_test(
                    ...
                    True,
                    f"{data.get('count', 0)} sources available",
                )
            else:
...
async def test_data_flows():
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            r = await client.post(
                ...
                json={"role": "user", "text": "Test message"},
            )
            if r.status_code == 200:
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            # Get first source
            r = await client.get(
                "http://localhost:8002/api/intelligence/sources?category=&subcategory="
            )
            if r.status_code == 200:
                ...
                if sources:
                    ...
                    r2 = await client.get(
                        f"http://localhost:8002/api/intelligence/preview/{source_id}"
                    )
                    if r2.status_code == 200:
                        ...
                        log_test(
                            ...
                            True,
                            f"Loaded preview for source {source_id}",
                        )
                    else:
                        log_test(
                            "Intelligence Preview", False, f"Status {r2.status_code}"
                        )
                else:
    ...
    if os.path.exists("main.py"):
        ...
        if os.path.exists("mcp.json"):
            ...
            with open("mcp.json") as f:
                ...
                log_test(
                    "MCP Tools Defined",
                    len(config.get("tools", [])) > 0,
                    f"{len(config.get('tools', []))} tools",
                )
        else:
...
def test_file_structure():
    ...
    required_files = [
        ...
        "dashboard_api.py",
        "intelligence_api.py",
        "command_center_spa.html",
        ".gitignore",
        "scripts/init_db.py",
        "scripts/seed_accounts.py",
        "scripts/analyze_intelligence.py",
    ]
