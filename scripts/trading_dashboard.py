...
def compare_performance():
    ...
    print(
        f"{'Metric':<25} {'AI Automated':<25} {'Human Manual':<25} {'Hybrid Partnership':<25}"
    )
    print("-" * 100)
    ...
    for metric_name, key, fmt in metrics:
        ...
        print(
            f"{metric_name:<25} {values[0]:<25} {values[1] if len(values) > 1 else 'N/A':<25} {values[2] if len(values) > 2 else 'N/A':<25}"
        )
...
def main_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            os.system(
                f"python scripts/ai_auto_trader.py --account-id 1 --loop --interval {interval}"
            )
