...
def show_predictions():
    ...
    for asset_type, preds in by_type.items():
        print(f"\n{asset_type.upper()}:")
        print(f"{'ID':<5} {'Asset':<8} {'Dir':<6} {'Conf':<6} {'Price':<12} {'Target Date':<12} {'Rationale'}")
        print(f"{'-'*90}")
        ...
        for pred in preds:
            ...
            print(f"{pred['id']:<5} {asset:<8} {pred['predicted_direction']:<6} "
...
def show_open_positions(account_id: int):
    ...
    print(f"{'='*90}\n")
    print(f"{'ID':<5} {'Asset':<8} {'Dir':<6} {'Entry':<12} {'Current':<12} {'Qty':<12} {'P&L %':<10}")
    print(f"{'-'*90}")
    
    for pos_id, asset, direction, entry_price, quantity, pos_size, opened_at in positions:
        ...
        current_str = f"${current_price:,.2f}" if current_price > 1 else f"${current_price:.6f}"
        
        pnl_color = '🟢' if pnl_pct > 0 else '🔴' if pnl_pct < 0 else '⚪'
        
        print(f"{pos_id:<5} {asset:<8} {direction:<6} {entry_str:<12} {current_str:<12} "
