...
def discover_fields(report_objs):
    ...
    for name, obj in report_objs:
        if isinstance(obj, dict):
            ...
            signals = obj.get('signals') or (obj.get('content') or {}).get('signals') if isinstance(obj.get('content'), dict) else obj.get('signals')
...
def main():
    ...
    wb.save(OUT_XLSX)
    print(f'Wrote {OUT_XLSX} with {len(report_objs)} reports and {total_signals} signals')
