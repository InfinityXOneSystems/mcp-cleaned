...
class HybridTrader:
    ...
    def calculate_ai_position_size(
        self, confidence: float, account_value: float
    ) -> float:
        """AI calculates recommended position size"""
        kelly_fraction = 0.5
        allocation_pct = (
            (confidence / 100) * kelly_fraction * self.ai_position_size_base
        )

        position_size = account_value * allocation_pct
        position_size = max(
            100, min(position_size, account_value * 0.08)
        )  # $100 min, 8% max
    ...
    def present_ai_recommendations(self):
        ...
        if alerts:
            ...
            for alert in alerts:
                ...
                print(
                    f"{severity_icon} Position #{alert['position_id']} - {alert['asset']}"
                )
            ...
            for alert in alerts:
                ...
                if decision == "close":
                    ...
                    close_position(
                        alert["position_id"], alert["current_price"], reason, "hybrid"
                    )
        ...
        if opportunities:
            ...
            for i, pred in enumerate(opportunities[:5], 1):  # Show top 5
                asset = pred["asset"]
                price = prices[asset]
                recommended_size = self.calculate_ai_position_size(
                    pred["confidence"], summary["total_value"]
                )
                ...
                print(
                    f"   Recommended Size: ${recommended_size:,.2f} ({(recommended_size/summary['total_value']*100):.1f}% of portfolio)"
                )
            ...
            for pred in opportunities[:5]:
                ...
                print(
                    f"   Confidence: {pred['confidence']:.0f}% | Size: ${recommended_size:,.2f}"
                )

                decision = (
                    input("   Your decision (approve/modify/reject): ").strip().lower()
                )
                ...
                if decision == "approve":
                    ...
                    custom_size = float(
                        input(
                            f"   Enter your position size (max ${summary['current_cash']:,.2f}): $"
                        )
                    )
