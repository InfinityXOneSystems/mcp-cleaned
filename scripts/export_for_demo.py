...
def export_results():
    ...
    for row in cur.fetchall():
        ...
        try:
            ...
            data_by_namespace[namespace].append({"url": key, "content": value})
        except:
            pass
    ...
    output["top_sources"] = [
        {"namespace": ns, "url_count": count} for ns, count in top_sources
    ]
    ...
    print(
        f"✓ Exported {output['summary']['total_urls_crawled']} URLs across {output['summary']['total_namespaces']} sources"
    )
    print(
        f"✓ Categories: {', '.join(f'{k}={len(v)} sources' for k,v in categories.items() if v)}"
    )
