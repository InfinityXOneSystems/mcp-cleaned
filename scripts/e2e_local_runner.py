...
def latest_snapshot_path():
    ...
    if FALLBACK_REPORT.exists():
        files = sorted(FALLBACK_REPORT.glob('*.json'), key=lambda p: p.stat().st_mtime, reverse=True)
        if files:
            return files[0]
    return None
...
async def run():
    ...
    if not path:
        print('No snapshot found in crawler/output/raw or data/reports. Run a crawl first.')
        return
    ...
    keywords = ["need funding fast","behind on payroll","pre foreclosure","tax delinquent"]
    ...
    for s in signals:
        ...
        fs.write_memory({'session_hash': f'e2e_{path.stem}_{sig["keyword"]}', 'type': 'signal', 'content': record})
