...
def discover_fields(report_objs):
    ...
    for repo_path, name, obj in report_objs:
        if isinstance(obj, dict):
            ...
            candidate_paths = [obj.get('signals'), (obj.get('content') or {}).get('signals') if isinstance(obj.get('content'), dict) else None, obj.get('items'), obj.get('results'), obj.get('payload')]
            signals = None
...
def main():
    ...
    for repo_path, name, obj in all_reports:
        ...
        if isinstance(obj, dict):
            candidates = [obj.get('signals'), (obj.get('content') or {}).get('signals') if isinstance(obj.get('content'), dict) else None, obj.get('items'), obj.get('results'), obj.get('payload')]
            for c in candidates:
                if isinstance(c, list):
                    signals = c
                    break
        if signals:
            for s in signals:
                if isinstance(s, dict):
                    flat_s = flatten(s)
                    row = [repo_path, name] + [ to_cell(flat_s.get(k,'')) for k in signal_keys ]
                    ws_s.append(row)
                    total_signals += 1

    wb.save(OUT_XLSX)
    print(f'Wrote {OUT_XLSX} with {len(all_reports)} reports from {root} and {total_signals} signals')
