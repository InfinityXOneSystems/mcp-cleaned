...
CATEGORY_KEYWORDS = {
    'distressed_properties': ['pre foreclosure','foreclosure','tax delinquent','probate','vacant house','code violation','delinquent tax'],
    'business_loans': ['business loan','cash flow','payroll','working capital','invoice','bridge loan','merchant cash advance','SBA','invoice factoring','accounts receivable'],
    'personal_loans': ['personal loan','paycheck advance','personal finance','installment loan','ppf','personal credit'],
    'asset_predictions': ['asset prediction','price drop','vacancy','market signal','insurance claim','price forecast','asset value']
}
    ...
    for rpt_dir in find_report_dirs(root):
        ...
        for f in rpt_dir.glob('*.json'):
            ...
            if isinstance(obj, dict):
                text = obj.get('text') or obj.get('html') or (obj.get('content') or {}).get('text') or json.dumps(obj)
            category = classify_text(text)
            dest_dir = OUT_ROOT / category / repo_name
            dest_dir.mkdir(parents=True, exist_ok=True)
            dest_path = dest_dir / f.name
            shutil.copy2(f, dest_path)
            index_rows.append({'repo_path': str(rpt_dir.parent), 'report_filename': f.name, 'category': category, 'dest_path': str(dest_path)})
    # write index
    idx_path = OUT_ROOT / 'index.csv'
    with idx_path.open('w', newline='', encoding='utf-8') as csvf:
        writer = csv.DictWriter(csvf, fieldnames=['repo_path','report_filename','category','dest_path'])
        writer.writeheader()
        for r in index_rows:
            writer.writerow(r)
