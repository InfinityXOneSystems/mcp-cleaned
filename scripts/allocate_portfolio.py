...
def calculate_allocation(total_capital: float = 5000.0):
    ...
    for asset_type, pct_allocation in type_allocation.items():
        ...
        for pred in type_predictions:
            ...
            confidence_weight = (
                pred["confidence"] / total_confidence
                if total_confidence > 0
                else 1.0 / len(type_predictions)
            )
...
def execute_allocation(
    account_id: int, allocations: list, execution_method: str = "ai_auto"
):
    ...
    print(f"\n{'='*80}")
    print(
        f"🤖 EXECUTING ALLOCATION FOR ACCOUNT {account_id} ({execution_method.upper()})"
    )
    print(f"{'='*80}\n")
...
def print_allocation_plan(allocations: list):
    ...
    print(
        f"{'Asset':<8} {'Type':<15} {'Dir':<6} {'Conf':<6} {'Amount':<12} {'%':<8} {'Rationale'}"
    )
    print(f"{'-'*80}")

    for a in allocations:
        print(
            f"{a['asset']:<8} {a['asset_type']:<15} {a['direction']:<6} "
    ...
    print(f"{'-'*80}")
    print(
        f"{'TOTAL':<8} {'':<15} {'':<6} {'':<6} ${total:>9,.2f}  {(total/5000*100):>6.2f}%"
    )
    print(f"{'='*80}\n")
