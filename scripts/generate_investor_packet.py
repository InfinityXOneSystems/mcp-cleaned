...
def generate_packet(staging_dir, report):
    ...
    lines.append("\n## Executive Summary")
    lines.append(
        "The `mcp` Omni Gateway is a self-sustaining, FAANG-grade orchestration hub that provides a single pane of control for predictions, crawling, and autonomous task execution. It is currently configured in `safe` doc-evolution mode to avoid any external writes."
    )

    # Scrape highlights placeholder
    lines.append("\n## Last Night — Scrape Highlights")
    lines.append(
        "- Sources scraped: *PLACEHOLDER* (use the staging snapshots to populate)"
    )
    ...
    lines.append("\n## Predictions Summary")
    lines.append(
        "- Prediction system: Unified predictor across Intelligence and Meta services"
    )
    lines.append("- Recent predictions: *PLACEHOLDER*")
    lines.append(
        "- Confidence model: calibrated to historical signals; see `scripts/` for calibration outputs"
    )

    # Capabilities
    lines.append("\n## System Capabilities (Highlights)")
    lines.append(
        "- Omni Gateway: Unified `predict`, `crawl`, `simulate`, `read`, `write`, `analyze` endpoints"
    )
    lines.append(
        "- Autonomous pipelines: agent-driven Auto-Builder, Vision Cortex integration (read-only)"
    )
    lines.append("- Doc Evolution: safe/read-only/live modes with audit gating")
    lines.append(
        "- Local memory: `mcp_memory.db` (SQLite) with optional external memory gateways"
    )
    lines.append(
        "- Admin Console: `http://localhost:8000/admin` for monitoring, toggles, and chat"
    )
    ...
    lines.append("\n## Next Steps")
    lines.append(
        "- Review consolidated snapshots, then allow human review to enable doc-evolution `read-only` mode."
    )
