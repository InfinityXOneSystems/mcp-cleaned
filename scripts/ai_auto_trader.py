...
from paper_trading import open_position, close_position, get_account_summary
...
class AITrader:
    ...
    def manage_risk(self, position: dict, current_price: float) -> str:
        ...
        if pnl_pct <= self.stop_loss_pct:
            return f"STOP_LOSS: Hit {self.stop_loss_pct}% stop loss (P&L: {pnl_pct:.2f}%)"
        
        # Take profit
        if pnl_pct >= self.take_profit_pct:
            return f"TAKE_PROFIT: Hit {self.take_profit_pct}% target (P&L: {pnl_pct:.2f}%)"
    ...
    def execute_strategy(self, dry_run: bool = False):
        ...
        if open_positions:
            ...
            for pos_id, asset, direction, entry_price, quantity, pos_size in open_positions:
        ...
        available_predictions = [p for p in predictions if p['asset'] not in open_assets]
        ...
        if available_predictions and current_position_count < self.max_positions:
            ...
            for pred in available_predictions[:5]:  # Check top 5 opportunities
                ...
                price = prices[asset]
                position_size = self.calculate_position_size(pred['confidence'], summary['current_cash'])
                
                if position_size > summary['current_cash']:
                    print(f"  ⚠️  {asset}: Insufficient cash (need ${position_size:.2f}, have ${summary['current_cash']:.2f})")
                    continue
                
                print(f"  🟢 OPPORTUNITY: {asset} @ ${price:.2f}")
                print(f"     Direction: {pred['predicted_direction'].upper()}")
                print(f"     Confidence: {pred['confidence']:.0f}%")
                print(f"     Position Size: ${position_size:.2f} ({(position_size/summary['total_value']*100):.1f}% of portfolio)")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dry-run', action='store_true', help='Simulation mode - no actual trades')
