...
class AsyncCrawler:
    def __init__(
        ...
        self.allowed_domains = (
            set(filter_allowed_domains(allowed_domains)) if allowed_domains else set()
        )
    ...
    async def fetch(self, session, url):
        ...
        try:
            async with session.get(
                url, timeout=ClientTimeout(total=20), headers=headers
            ) as resp:
            ...
            async with self.sem:
                ...
                if html:
                    ...
                    if depth < self.max_depth:
                        for l in links:
                            if (
                                l not in self.visited
                                and self.to_visit.qsize() + len(self.results)
                                < self.max_pages
                            ):
    ...
    async def crawl(self, start_url):
        ...
        if not self.allowed_domains:
            ...
            self.allowed_domains = set(filter_allowed_domains(self.allowed_domains)) | {
                start_host
            }
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            tasks = [
                asyncio.create_task(self.worker(session)) for _ in range(self.max_tasks)
            ]
