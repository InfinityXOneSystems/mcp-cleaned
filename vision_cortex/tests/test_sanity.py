...
def _registry() -> MemoryRegistry:
    return MemoryRegistry(firestore=InMemoryFirestore(), vector_store=InMemoryVectorStore())
...
def test_daily_ingestion_pipeline_runs() -> None:
    ...
    ingestor = IngestorAgent(name="ingestor_01", role="ingestor", bus=bus, memory=registry)
    organizer = OrganizerAgent(name="organizer_01", role="organizer", bus=bus, memory=registry)
    validator = ValidatorAgent(name="validator_01", role="validator", bus=bus, memory=registry)
    pipeline = DailyIngestionPipeline(crawler, ingestor, organizer, validator)

    result = pipeline.run(
        session_id="sess-1",
        task_id="task-1",
        seed={"sources": [{"title": "AI model release", "url": "https://example.com", "content": "New AI model released"}]},
    )
...
def test_debate_cycle_produces_document() -> None:
    bus = MessageBus()
    registry = _registry()
    predictor = PredictorAgent(name="predictor_01", role="predictor", bus=bus, memory=registry)
    visionary = VisionaryAgent(name="visionary_01", role="visionary", bus=bus, memory=registry)
    strategist = StrategistAgent(name="strategist_01", role="strategist", bus=bus, memory=registry)
    validator = ValidatorAgent(name="validator_01", role="validator", bus=bus, memory=registry)
    documentor = DocumentorAgent(name="documentor_01", role="documentor", bus=bus, memory=registry)

    debate = DebateCycle(predictor, visionary, strategist, validator, documentor)
    organized = {"clusters": {"ai": [{"title": "AI release", "text": "increase in ai capability"}]}}
...
def test_evolver_generates_improvements() -> None:
    ...
    result = evolver.run_task(ctx, {"validations": {"risks": [{"type": "test"}], "contradictions": []}})
