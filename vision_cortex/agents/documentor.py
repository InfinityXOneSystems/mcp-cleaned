...
class DocumentorAgent(BaseAgent):
    ...
    def _build_report(self, predictions: List[Dict[str, Any]], steps: List[Dict[str, Any]], actions: List[Dict[str, Any]]) -> str:
        lines: List[str] = []
        if predictions:
            lines.append("Predictions:")
            for pred in predictions:
                lines.append(f"- ({pred.get('tag','')}) {pred.get('statement')} [conf={pred.get('confidence',0):.2f}]")
        if steps:
            lines.append("\nPlans:")
            for step in steps:
                actions_text = "; ".join(step.get("actions", []))
                lines.append(f"- {step.get('timeframe')}: {actions_text} (risk={step.get('risk')})")
        if actions:
            lines.append("\nPriorities:")
            for act in actions:
