...
class DailyIngestionPipeline:
    ...
    def run(
        self, session_id: str, task_id: str, seed: Dict[str, Any]
    ) -> Dict[str, Any]:
        ctx = AgentContext(
            session_id=session_id, task_id=task_id, governance_level="LOW"
        )
        ...
        validated = self._step(
            "validator", self.validator, ctx, {"organized": organized}
        )
        return {
    ...
    def _step(
        self, name: str, agent: BaseAgent, ctx: AgentContext, payload: Dict[str, Any]
    ) -> Any:
        agent.log_event(
            f"Executing step {name}", ctx, {"payload_keys": list(payload.keys())}
        )
