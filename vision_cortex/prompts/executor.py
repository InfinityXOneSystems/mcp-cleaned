...
from vision_cortex.prompts.registry import PromptDefinition, resolve_alias
from vision_cortex.validators.safety import enforce_governance


class PromptExecutor:
    def __init__(self, bus: MessageBus, memory: Optional[MemoryRegistry] = None) -> None:
        self.bus = bus
        self.memory = memory
        self.orchestrator = SystemBuildOrchestrator(bus=bus, memory=memory)

    def execute(self, prompt_or_alias: str, params: Optional[Dict[str, Any]] = None, confidence: float = 0.5) -> Dict[str, Any]:
