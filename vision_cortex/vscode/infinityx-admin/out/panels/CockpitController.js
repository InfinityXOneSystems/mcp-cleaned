...
exports.CockpitController = void 0;
...
class CockpitController {
    ...
    show() {
        ...
        this.panel = vscode.window.createWebviewPanel("infinityxCockpit", "Intelligence Cockpit", vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        this.panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                case "getPipelineStatus":
                    ...
                    break;
                case "invokeAgent":
                    await this.invokeAgent(message.payload?.agent, message.payload?.action, message.payload?.data);
                    break;
                case "refresh":
     ...
     */
    async runBuilderCycle(mode) {
        this.postMessage({ command: "loading", payload: { message: "Starting build cycle..." } });
        try {
            const result = await (0, mcpClient_1.mcpRunPipeline)(mode);
    ...
    async getPipelineStatus() {
        try {
            const status = await (0, mcpClient_1.mcpPipelineStatus)();
     ...
     */
    async invokeAgent(agent, action, data) {
        this.postMessage({ command: "loading", payload: { message: `Invoking ${agent}...` } });
        try {
            const result = await (0, mcpClient_1.mcpInvoke)(agent, action, data);
...
exports.CockpitController = CockpitController;
//# sourceMappingURL=CockpitController.js.map