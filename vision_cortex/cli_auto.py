...
from vision_cortex.prompts import ALIASES, list_prompts, resolve_alias
from vision_cortex.prompts.executor import PromptExecutor

# Try to import domain-specific utilities
try:
    from vision_cortex.prompts.domain_registry import get_domain_categories, list_domain_prompts
    HAS_DOMAIN = True
except ImportError:
    HAS_DOMAIN = False
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Vision Cortex Auto-Build & Prompt CLI")
    parser.add_argument("--runs", type=int, default=1, help="Number of builds to run in parallel")
    parser.add_argument("--governance", type=str, default="HIGH", help="Governance level (LOW|MEDIUM|HIGH|CRITICAL)")
    parser.add_argument("--seed", type=str, default="{}", help="JSON seed payload")
    parser.add_argument("--prompt", "-p", type=str, help="Execute prompt by alias or ID (e.g., scan, brand, L2_MARKET_ANALYSIS)")
    parser.add_argument("--confidence", "-c", type=float, default=0.75, help="Confidence level for prompt execution")
    ...
    parser.add_argument("--category", type=str, help="Filter prompts by category (business, system, personal, docs, analysis, workflow, governance, special)")
    parser.add_argument("--categories", action="store_true", help="List available prompt categories")
    ...
    if args.list:
        ...
        if args.category:
            prompts = [p for p in prompts if args.category.lower() in (t.lower() for t in p.tags)]
    ...
    if args.prompt:
        ...
        try:
            ...
            params = None
        result = executor.execute(args.prompt, params=params, confidence=args.confidence)
        print(json.dumps(result, indent=2, default=str))
        return 0 if result.get("status") != "error" else 1

    # Build mode (original behavior)
    orchestrator = SystemBuildOrchestrator(bus=bus, memory=memory, governance_level=args.governance)
