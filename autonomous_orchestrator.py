...
async def start_agent(config: AgentConfig, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(
        run_agent_loop, config.agent_id, config.config, config.interval_seconds
    )

    logger.info(f"Started agent: {config.agent_id}")

    return JSONResponse(
        content={"success": True, "agent_id": config.agent_id, "status": "running"}
    )
...
async def stop_agent(agent_id: str):
    ...
    return JSONResponse(
        content={"success": True, "agent_id": agent_id, "status": "stopped"}
    )
...
async def list_agents():
    ...
    return JSONResponse(content={"success": True, "agents": ACTIVE_AGENTS})
...
async def create_trigger(trigger: TriggerConfig):
    ...
    try:
        ...
        client.collection("mcp_memory").document(f"trigger_{trigger.trigger_id}").set(
            doc
        )
...
async def execute_trigger(trigger_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not trigger_data.get("enabled", False):
            return JSONResponse(
                content={"success": False, "message": f"Trigger disabled: {trigger_id}"}
            )
        ...
        for agent_id in agent_ids:
            try:
                ...
                results.append(
                    {"agent_id": agent_id, "status": "error", "error": str(e)}
                )
