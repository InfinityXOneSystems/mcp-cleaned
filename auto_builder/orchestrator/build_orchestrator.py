...
class BuildPlan:
    ...
    live_execution_requires: List[str] = field(default_factory=lambda: [
    ...
    def from_json(cls, data: Dict[str, Any]) -> "BuildPlan":
        ...
        return cls(
            ...
            live_execution_requires=data.get("live_execution_requires", [
                "validation_report.confidence >= governance_threshold",
                "validator.approval == true"
            ])
        )
...
class BuildOrchestrator:
    ...
    def _validate_plan(self, plan: BuildPlan) -> List[str]:
        ...
        if plan.requested_by != "VisionCortex":
            violations.append(f"Invalid requester: {plan.requested_by} (must be VisionCortex)")
        ...
        # Check artifacts
        required_artifacts = ["build_plan.json", "execution_log.json", 
    ...
    async def execute(self, dry_run: bool = True) -> Dict[str, Any]:
        ...
        if self.validator_gate:
            ...
            if not pre_result["approved"]:
                ...
                return {"success": False, "reason": "Pre-execution validation failed", **pre_result}
        ...
        for i, step in enumerate(self.context.plan.steps):
            ...
            try:
                if self.step_executor:
                    ...
                    result = {"step_id": step.get("step_id"), "status": "skipped", "reason": "No executor"}
        ...
        if self.validator_gate:
            post_result = await self.validator_gate.post_execution_check(self.context, results)
            if not post_result["approved"]:
                self.context.state = BuildState.FAILED
                self._log("post_validation_failed", post_result)
                return {"success": False, "reason": "Post-execution validation failed", **post_result}
        ...
        self._log("execution_completed", {"steps": len(results), "errors": len(self.context.errors)})
