...
class ValidatorGate:
    ...
    async def pre_execution_check(self, context: Any) -> Dict[str, Any]:
        ...
        checks.append(
            ValidationCheck(
                ...
                message=(
                    f"Unauthorized paths: {path_violations}"
                    if path_violations
                    else "All paths authorized"
                ),
            )
        )
        ...
        report = ValidationReport(
            result=(
                ValidationResult.APPROVED if all_passed else ValidationResult.REJECTED
            ),
            checks=checks,
    ...
    async def post_execution_check(
        self, context: Any, step_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        checks.append(
            ValidationCheck(
                ...
                message=(
                    f"{len(context.errors)} errors" if context.errors else "No errors"
                ),
            )
        )
        ...
        checks.append(
            ValidationCheck(
                ...
                message=(
                    f"Unauthorized changes: {unauthorized}"
                    if unauthorized
                    else "All changes authorized"
                ),
            )
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_validation": (
                self._validation_history[-1].to_dict()
