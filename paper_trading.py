...
def create_account(
    ...
    try:
        ...
        conn.commit()
        print(f"✓ Account created: {name} (ID: {account_id}) - ${starting_balance:,.2f}")
        return account_id
...
def open_position(
    ...
    # Get account balance
    cur.execute("SELECT current_balance FROM paper_accounts WHERE id = ?", (account_id,))
    ...
    cur.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open', ?)
    """, (account_id, asset, asset_type, direction, entry_price, position_size, quantity, now, entry_reason))
    ...
    cur.execute("""
        ...
        VALUES (?, ?, 'open', ?, ?, ?, ?, ?, ?, ?)
    """, (account_id, position_id, asset, entry_price, quantity, position_size, now, execution_method, entry_reason))
    
    # Update account balance
    new_balance = balance - position_size
    cur.execute("UPDATE paper_accounts SET current_balance = ?, updated_at = ? WHERE id = ?", (new_balance, now, account_id))
    
    conn.commit()
    conn.close()
    
    print(f"✓ Opened {direction.upper()} position: {asset} @ ${entry_price:.2f} × {quantity:.4f} = ${position_size:,.2f}")
    return position_id
...
def close_position(
    ...
    cur.execute("""
        INSERT INTO paper_trades (account_id, position_id, trade_type, asset, price, quantity, value, executed_at, execution_method, notes)
        VALUES (?, ?, 'close', ?, ?, ?, ?, ?, ?, ?)
    """, (account_id, position_id, asset, exit_price, quantity, exit_value, now, execution_method, exit_reason))
    ...
    print(f"✓ Closed position: {asset} @ ${exit_price:.2f} | P&L: ${pnl:+,.2f} ({pnl_pct:+.2f}%)")
    return pnl
...
def print_account_summary(account_id: int):
    ...
    if summary['open_positions']:
        ...
        for pos in summary['open_positions']:
            ...
            print(f"  {direction.upper():<6} {asset:<8} @ ${entry:>8.2f} × {qty:>10.4f} = ${size:>10,.2f}")
