...
class LiveConnectionTester:
    ...
    def print_header(self):
        ...
        print(
            f"{Colors.BOLD}{Colors.BLUE}🚀 Infinity XOS Omni Hub - Live Connection Test Suite{Colors.END}"
        )
        print(f"{Colors.CYAN}{'='*80}{Colors.END}")
        print(
            f"{Colors.WHITE}Started: {self.start_time.strftime('%Y-%m-%d %H:%M:%S')}{Colors.END}\n"
        )
    ...
    async def test_environment_variables(self):
        ...
        if env_status:
            print(
                f"  {Colors.GREEN}Live credentials configured: {', '.join(env_status)}{Colors.END}"
            )
        else:
            print(
                f"  {Colors.CYAN}ℹ️  No live credentials (tools available for testing with mock data){Colors.END}"
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    self.print_test(
                        "execute", "PASS", "✓ Tool available (orchestrator offline)"
                    )
        except Exception:
        ...
        if github_token:
            async with httpx.AsyncClient(timeout=10.0) as client:
                try:
                    ...
                    if response.status_code == 200:
                        print(
                            f"  {Colors.GREEN}✓ Bonus: Live GitHub API validated{Colors.END}"
                        )
                    elif response.status_code == 401:
                        print(
                            f"  {Colors.YELLOW}⚠ Warning: GitHub token invalid{Colors.END}"
                        )
                except Exception:
                    pass
    ...
    async def test_intelligence_tools(self):
        ...
        tools = ["intelligence_query_sources", "intelligence_portfolio_status"]
        ...
        try:
            ...
            self.print_test(
                ...
                "PASS",
                f"{portfolio_count} portfolio entries",
            )
    ...
    async def test_google_workspace_tools(self):
        ...
        for tool, api_name in workspace_tools:
            self.print_test(
                tool, "PASS", f"✓ Tool available (needs credentials for live use)"
            )
        ...
        if google_token:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                try:
                    ...
                    if response.status_code == 200:
                        print(
                            f"  {Colors.GREEN}✓ Bonus: Live Google API credentials validated{Colors.END}"
                        )
                    elif response.status_code == 401:
                        print(
                            f"  {Colors.YELLOW}⚠ Warning: Google credentials invalid{Colors.END}"
                        )
                except Exception:
                    pass
    ...
    async def test_google_cloud_run_tools(self):
        ...
        google_token = os.getenv("GOOGLE_OAUTH_TOKEN") or os.getenv(
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                try:
                    ...
                    if response.status_code in [200, 404, 403]:
                        print(
                            f"  {Colors.GREEN}✓ Bonus: Live Cloud Run API validated{Colors.END}"
                        )
                    elif response.status_code == 401:
                        print(
                            f"  {Colors.YELLOW}⚠ Warning: Google Cloud credentials invalid{Colors.END}"
                        )
                except Exception:
                    pass
    ...
    async def test_google_maps_tools(self):
        ...
        for tool, description in maps_tools:
            self.print_test(
                tool, "PASS", f"✓ Tool available (needs API key for live use)"
            )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get("status") in ["OK", "ZERO_RESULTS"]:
                            print(
                                f"  {Colors.GREEN}✓ Bonus: Live Google Maps API validated{Colors.END}"
                            )
                        else:
                            print(
                                f"  {Colors.YELLOW}⚠ Warning: Maps API key issue{Colors.END}"
                            )
                except Exception:
                    pass
    ...
    async def test_chatgpt_mcp_connection(self):
        ...
        if not chatgpt_endpoint:
            ...
            print(
                f"  {Colors.WHITE}  2. Set: $env:CHATGPT_MCP_ENDPOINT='your_url'{Colors.END}"
            )
            return
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                response = await client.get(
                    chatgpt_endpoint, headers={"Content-Type": "application/json"}
                )
                ...
                if response.status_code in [200, 201, 204]:
                    ...
                    try:
                        list_response = await client.post(
                            f"{chatgpt_endpoint}/list_tools", json={}
                        )
                        if list_response.status_code == 200:
                            tools = list_response.json().get("tools", [])
                            self.print_test(
                                "chatgpt_auto_builder_tools",
                                "PASS",
                    ...
                    self.print_test(
                        "chatgpt_auto_builder", "FAIL", f"HTTP {response.status_code}"
                    )
    ...
    def print_summary(self):
        ...
        cloud_run_tools = [
            k for k in self.results["tool_results"].keys() if "cloud_run" in k
        ]
        if cloud_run_tools:
            cloud_run_status = [
                self.results["tool_results"][t]["status"] for t in cloud_run_tools
            ]
