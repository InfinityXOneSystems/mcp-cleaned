...
async def run_shadow_agent(
    ...
    print(
        f"[{name}] starting crawl {start_url} (concurrency={concurrency}, max_pages={max_pages})"
    )
    ...
    try:
        if use_playwright:
            ...
            return {
                "name": name,
                "result": {"pages_crawled": 1, "start_url": start_url},
            }
...
async def run_team(seeds: List[str], concurrency: int = 5, max_pages: int = 20):
    ...
    for i, s in enumerate(seeds):
        tasks.append(
            run_shadow_agent(
                f"agent_{i+1}", s, concurrency=concurrency, max_pages=max_pages
            )
        )
...
def main():
    ...
    parser.add_argument(
        "--no-robots",
        action="store_true",
        help="Ignore robots.txt (requires --dev-ok or ALLOW_NO_ROBOTS=1)",
    )
    parser.add_argument(
        "--dev-ok",
        action="store_true",
        help="Acknowledges you are in a dev/testing environment (required to use --no-robots)",
    )
    parser.add_argument(
        "--enable-all-capabilities",
        action="store_true",
        help="Enable all optional/testing capabilities (Playwright, tracing, metrics). May attempt to auto-install if --auto-install is provided",
    )
    parser.add_argument(
        "--auto-install",
        action="store_true",
        help="If capability packages are missing, attempt to install them automatically (requires network).",
    )
    ...
    if args.enable_all_capabilities:
        if not PLAYWRIGHT_AVAILABLE:
            if args.auto_install:
                print(
                    "Playwright not found — attempting to install via pip. This requires network access and may take a while."
                )
                try:
                    import subprocess
                    ...
                    try:
                        ...
                        print(
                            "Warning: playwright browser install step failed or requires extra privileges. Continue and try manual install if needed."
                        )
                except Exception as e:
                    print(f"Auto-install failed: {e}")
                    print(
                        "Proceeding without Playwright. If you need Playwright, run: pip install playwright && python -m playwright install"
                    )
            else:
                print(
                    "Note: Playwright not installed. To enable Playwright pass --auto-install or install manually:"
                )
    ...
    if args.no_robots and not (args.dev_ok or allow_no_robots_env):
        ...
        print(
            "Either pass --dev-ok or set environment variable ALLOW_NO_ROBOTS=1 for testing in controlled environments."
        )
        sys.exit(1)
