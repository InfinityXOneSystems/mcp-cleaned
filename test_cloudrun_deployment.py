    ...
    def test(self, name: str, func):
        ...
        try:
            ...
            if result:
                ...
                self.failed += 1
                self.results.append(
                    {"test": name, "status": "FAIL", "error": "Test returned False"}
                )
...
def test_mcp_list_tools():
    ...
    resp = requests.get(
        f"{CLOUD_RUN_URL}/mcp/listMCPTools", headers=HEADERS, timeout=10
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_mcp_execute_safe_tool():
    ...
    payload = {"tool": "get_weather_data", "args": {"city": "New York"}}
    resp = requests.post(
        f"{CLOUD_RUN_URL}/mcp/executeMCPTool", headers=HEADERS, json=payload, timeout=15
    )
...
def test_credential_gateway_auth():
    ...
    resp = requests.get(
        f"{CLOUD_RUN_URL}/credentials/get/github_app",
...
def test_autonomy_agents_list():
    ...
    headers_with_token = {"Authorization": f"Bearer {MCP_API_KEY}"}
    resp = requests.get(
        f"{CLOUD_RUN_URL}/autonomy/agents", headers=headers_with_token, timeout=10
    )
    assert resp.status_code == 200
    data = resp.json()
    assert "agents" in data
...
def test_intelligence_sources():
    ...
    resp = requests.get(
        f"{CLOUD_RUN_URL}/intelligence/sources", headers=HEADERS, timeout=10
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_safe_mode_enforcement():
    ...
    payload = {"tool": "system_shutdown", "args": {}}  # Hypothetical dangerous tool
    resp = requests.post(
        f"{CLOUD_RUN_URL}/mcp/executeMCPTool", headers=HEADERS, json=payload, timeout=10
    )
...
def test_firestore_connectivity():
    ...
    resp = requests.post(
        f"{CLOUD_RUN_URL}/intelligence/query",
        headers=HEADERS,
...
def test_rate_limiting_headers():
    ...
    assert (
        "access-control-allow-origin" in resp.headers or "content-type" in resp.headers
    )
    return True
