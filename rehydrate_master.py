...
DEFAULT_GATEWAY_URL = (
    os.environ.get("GATEWAY_URL") or "https://gateway.infinityxoneintelligence.com"
)
...
def build_manifest() -> Dict[str, Any]:
    ...
    manifest = {
        ...
        "location_notes": "Primary cloud region: us-east1. Local workspace path: "
        + os.path.abspath(os.path.dirname(__file__)),
        "how_it_works": (
        ...
        "attached_apps": [
            {"name": "Frontend Service", "url": DEFAULT_GATEWAY_URL},
            {
                ...
                "url": "https://gateway-896380409704.us-east1.run.app",
            },
        ...
        "folder_paths": {
            ...
            "credential_manager": os.path.expandvars(
                r"%USERPROFILE%\\AppData\\Local\\InfinityXOne\\CredentialManager"
            ),
        },
    ...
    try:
        boot_prompt_path = os.path.join(
            os.path.dirname(__file__), "BOOT_AGENT_PROMPT.txt"
        )
...
def write_local_manifest(
    manifest: Dict[str, Any], path: str = LOCAL_MANIFEST_PATH
) -> None:
...
def init_firestore_client() -> "firestore.Client | None":
    if not GCLOUD_AVAILABLE:
        logger.warning(
            "google-cloud package not available; Firestore operations disabled"
        )
        return None
    try:
...
def write_manifest_to_firestore(
    manifest: Dict[str, Any], client: "firestore.Client"
) -> bool:
    try:
        doc_ref = client.collection(FIRESTORE_COLLECTION).document(
            "rehydrate_master_manifest"
        )
        doc_ref.set(manifest)
        logger.info(
            "Manifest written to Firestore/%s/rehydrate_master_manifest",
...
def update_protocol_checklist(client: "firestore.Client", mark: Dict[str, str]) -> bool:
    try:
        ...
        if not doc.exists:
            logger.warning(
                "protocol_110 not found in Firestore; skipping checklist update"
            )
            return False
        data = doc.to_dict() or {}
...
def main():
    parser = argparse.ArgumentParser(
        description="Rehydrate master manifest and optionally persist to Firestore"
    )
    parser.add_argument(
        "--write-firestore", action="store_true", help="Write manifest to Firestore"
    )
    parser.add_argument(
        "--update-checklist",
        action="store_true",
        help="Update protocol_110 checklist in Firestore based on checks",
    )
    parser.add_argument(
        "--gateway-url", default=DEFAULT_GATEWAY_URL, help="Gateway URL to check"
    )
