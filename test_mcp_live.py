...
class TestMCPLiveTester:
    ...
    async def call_mcp_tool(self, tool_name, args):
        ...
        try:
            ...
            response_line = await asyncio.wait_for(
                self.mcp_process.stdout.readline(), timeout=10.0
            )
    ...
    async def test_vscode_tools(self):
        ...
        tests = [
            {
                ...
                and ("results" in r or "matches" in r),
            },
            {
        ...
        for test in tests:
            try:
                ...
                self.log(
                    f"    [✓] PASS: {test['tool']} (Real MCP call succeeded)", "SUCCESS"
                )
    ...
    def generate_report(self):
        ...
        for category in self.results["categories_tested"]:
            ...
            successes = [
                t
                for t in self.results["tools_tested"]
                if t in self.results["successes"]
            ]
            if successes:
            ...
            failures = [
                ...
                if f["tool"] in self.results["tools_tested"]
            ]
            if failures:
        ...
        # Save to file
        report_file = (
            f"test_results_live_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
