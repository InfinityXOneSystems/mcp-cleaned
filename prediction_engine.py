...
def log_prediction(
    ...
    print(
        f"✓ Prediction #{prediction_id} logged: {asset} {predicted_direction or predicted_value} by {target_date} ({confidence}% confidence)"
    )
    return prediction_id


def resolve_prediction(
    prediction_id: int, actual_value: float = None, actual_direction: str = None
):
    ...
    cur.execute(
        "SELECT predicted_value, predicted_direction, confidence FROM predictions WHERE id = ?",
        (prediction_id,),
    )
    row = cur.fetchone()
    if not row:
    ...
    print(
        f"✓ Prediction #{prediction_id} resolved: {outcome} (score: {accuracy_score:.2f})"
    )
