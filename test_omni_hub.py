...
def main():
    ...
    try:
        ...
        print(f"{Colors.MAGENTA}  Architecture:{Colors.RESET} Multi-system orchestration with soft guardrails")
        ...
        for tool in TOOLS:
            ...
            elif tool.name.startswith("query_") or tool.name.startswith("get_portfolio"):
            ...
            elif tool.name.startswith("google_calendar") or tool.name.startswith("google_sheets") or \
                 tool.name.startswith("google_drive") or tool.name.startswith("google_gmail") or \
                 tool.name.startswith("google_docs_create"):
            ...
            elif tool.name.startswith("google_custom_search") or tool.name.startswith("google_analytics"):
            ...
            elif tool.name.startswith("google_recaptcha") or tool.name.startswith("google_translate"):
            ...
            elif tool.name.startswith("google_speech") or tool.name.startswith("google_text_to_speech") or \
                 tool.name.startswith("google_video"):
        ...
        for category in sorted(categories.keys()):
            ...
            if tools:
                total += len(tools)
                icon = "🔧" if category == "Orchestration" else \
                       "🐙" if category == "GitHub" else \
                       "🐳" if category == "Docker" else \
                       "🧠" if category == "Intelligence" else \
                       "🔵"
                print(f"{icon} {Colors.BOLD}{category}:{Colors.RESET} {len(tools)} tools")
        ...
        for capability, desc in capabilities.items():
            print(f"{Colors.GREEN}✓{Colors.RESET} {Colors.BOLD}{capability}{Colors.RESET}")
        ...
        print(f"  {Colors.YELLOW}GITHUB_TOKEN{Colors.RESET} .............. GitHub API authentication")
        print(f"  {Colors.YELLOW}GOOGLE_OAUTH_TOKEN{Colors.RESET} ........ Google API OAuth2 token")
        print(f"  {Colors.YELLOW}ORCHESTRATOR_URL{Colors.RESET} .......... Infinity XOS endpoint\n")
        ...
        print(f"{Colors.GREEN}{Colors.BOLD}✨ OMNI HUB v3.0 - 58 TOOLS - MAXIMUM CONNECTIVITY ✨{Colors.RESET}\n")
