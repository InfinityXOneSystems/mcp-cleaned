...
class SystemMonitor:
        ...
        for name, endpoint in endpoints.items():
            try:
                ...
                if response.status_code >= 400:
                    self.alerts.append(f'{name} endpoint returned {response.status_code}')
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {
                    ...
                    'changes_count': len(result2.stdout.strip().split('\n')) if has_changes else 0
                }
            return {'status': 'ERROR'}
        except Exception as e:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if builds:
                    ...
                    if status == 'FAILURE':
                        self.alerts.append(f'Build {latest.get("id", "unknown")} FAILED')
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        self.checks = {
            ...
            'overall_status': 'HEALTHY' if not self.alerts else 'DEGRADED' if len(self.alerts) < 3 else 'CRITICAL'
        }
        return self.checks
