...
def test_github_multi_account():
    ...
    try:
        ...
        print(
            f"   ✓ Personal: {p_rate['rate']['remaining']}/{p_rate['rate']['limit']} remaining"
        )
        print(
            f"   ✓ Organization: {o_rate['rate']['remaining']}/{o_rate['rate']['limit']} remaining"
        )
    except Exception as e:
        print(f"   ✗ Error: {e}")
...
def test_credential_sync():
    ...
    if config_path.exists():
        ...
        print(
            f"   ✓ Webhook Secret: {'***' if config.get('webhook_secret') else 'Not set'}"
        )
