...
class RehydrationExecutor:
    ...
    def phase1_verify_gateway(self) -> bool:
        ...
        if tools and "data" in tools:
            ...
            self._log(
                f"   Governance: LOW={governance.get('LOW')}, MEDIUM={governance.get('MEDIUM')}, HIGH={governance.get('HIGH')}, CRITICAL={governance.get('CRITICAL')}"
            )
    ...
    def phase2_initialize_memory(self) -> bool:
        ...
        memory = self._make_request(
            "GET", "/memory/query", params={"type": "system", "limit": 50}
        )
        ...
        for agent in genesis_agents:
            ...
            result = self._make_request(
                ...
                json={
                    ...
                    "arguments": {"agent_id": agent["id"], "config": agent_config},
            ...
            if result and result.get("status") == "success":
                ...
                self._log(
                    f"   ⚠️ {agent['name']} creation pending (endpoint may need implementation)"
                )
    ...
    def execute_full_rehydration(self) -> Dict[str, Any]:
        ...
        self._log("")
        self._log(
            f"Gateway Reachable: {'✅' if self.status['gateway_reachable'] else '❌'}"
        )
        self._log(
            f"Memory Connected: {'✅' if self.status['memory_connected'] else '❌'}"
        )
    ...
    def run_health_check(self) -> Dict[str, Any]:
        ...
        endpoints = ["/health", "/autonomy/health", "/langchain/health", "/mcp/tools"]
