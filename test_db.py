...
for row in cur.fetchall():
    print(
        f"ID: {row[0]} | Key: {row[1][:50]}... | Value length: {len(row[2]) if row[2] else 0} chars"
    )
...
cur.execute(
    "SELECT id, asset, direction, entry_price, quantity, status FROM paper_positions LIMIT 10"
)
print("\n=== Sample Positions ===")
for row in cur.fetchall():
    print(
        f"ID: {row[0]} | {row[1]} {row[2]} @ ${row[3]:.2f} x {row[4]:.4f} | Status: {row[5]}"
    )
