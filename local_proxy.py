...
async def host_router(request: Request, call_next):
    ...
    if path.startswith('/admin') or path.startswith('/api') or path.startswith('/sheets') or path.startswith('/calendar'):
        async with httpx.AsyncClient() as client:
            target = GATEWAY_URL + path
            try:
                if request.method in ('POST','PUT','PATCH'):
                    body = await request.body()
                    resp = await client.request(request.method, target, content=body, headers=request.headers)
                else:
                    resp = await client.request(request.method, target, params=dict(request.query_params), headers=request.headers)
