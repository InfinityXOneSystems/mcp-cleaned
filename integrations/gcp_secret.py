...
def fetch_secret_to_file(
    ...
    """
    try:
        from google.cloud import secretmanager  # pylint: disable=C0415
    except Exception as e:
        raise RuntimeError(f"google-cloud-secret-manager library is not installed: {e}") from e

    project = (
        project or os.environ.get("GCP_PROJECT") or os.environ.get("FIRESTORE_PROJECT")
    )
    if not project:
        raise RuntimeError(
            "GCP project not provided via argument or GCP_PROJECT / FIRESTORE_PROJECT env var"
        )
    ...
    try:
        ...
        raise RuntimeError(f"failed to access secret {name}: {e}") from e
    ...
    try:
        ...
        raise RuntimeError(f"failed to write secret to {out_dir}: {e}") from e
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out", help="Output file path (defaults to .secrets/<secret>.json)"
    )
    parser.add_argument("--version", default="latest")
    args = parser.parse_args()
    path = fetch_secret_to_file(
        args.secret, project=args.project, version=args.version, out_path=args.out
    )
    print(path)
