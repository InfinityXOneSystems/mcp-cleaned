...
def find_doc_evolve_file() -> Optional[str]:
    ...
    for base in EXTERNAL_PATHS:
        ...
        common = os.path.join(base, 'services', 'real-estate-intelligence', 'vision-cortex', 'doc_system', 'doc_evolve.py')
...
def _load_doc_evolver():
    ...
    try:
        ...
        if hasattr(mod, 'DocEvolveSystem'):
            ...
            if DOC_EV_MODE == 'safe':
                logger.info('DOC_EV_MODE=safe: not instantiating DocEvolveSystem to avoid external writes')
                return None
            _doc_evolver = DocEvolveSystem(versions_dir=versions_dir)
            logger.info('Loaded DocEvolveSystem from %s (mode=%s)', DOC_EV_FILE, DOC_EV_MODE)
            return _doc_evolver
    except Exception:
...
def ingest_document(source: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if evolver:
        try:
            ...
            change_summary = metadata.get('summary', 'ingest') if isinstance(metadata, dict) else 'ingest'
            author = metadata.get('author', 'system') if isinstance(metadata, dict) else 'system'
            if DOC_EV_MODE == 'read-only':
                # do not write; return what would have been done
                return {'status': 'read-only', 'doc_id': doc_id, 'would_create': True, 'change_summary': change_summary}
            if DOC_EV_MODE == 'live':
                version_id = evolver.create_version(doc_id, text, change_summary=change_summary, author=author)
            ...
            return {'status': 'skipped', 'reason': f'mode={DOC_EV_MODE}', 'doc_id': doc_id}
        except Exception:
            logger.exception('Evolver ingest failed')

    # Stubbed behavior: return small summary
    summary = text[:1000]
    return {'status': 'stubbed', 'summary': summary, 'source': source, 'metadata': metadata}
...
def transform_document(doc_id: str, operations: List[Dict[str, Any]]) -> Dict[str, Any]:
    logger.info('Transform doc: %s ops:%d', doc_id, len(operations) if operations else 0)
    evolver = _load_doc_evolver()
    if evolver:
            ...
            for op in operations:
                ...
                if t:
                    ...
                    if ok:
                        ...
                        vid = evolver.create_version(doc_id, improved, change_summary=f'applied:{t}', author=op.get('author', 'system'))
                        applied += 1
    ...
    return {'status': 'stubbed', 'doc_id': doc_id, 'applied': len(operations) if operations else 0}
    ...
    if evolver:
        try:
            ...
            if 'improvement_type' in strategy:
                ...
                ok, improved = evolver.apply_improvement(doc_id, content, strategy['improvement_type'])
                if ok:
                    vid = evolver.create_version(doc_id, improved, change_summary=f"evolved:{strategy['improvement_type']}")
                    return {'status': 'ok', 'version_id': vid}
                return {'status': 'error', 'reason': 'apply_failed'}

            return {'status': 'ok', 'message': 'no-op or unknown strategy', 'strategy': strategy}
        except Exception:
...
def sync_documents(target: str) -> Dict[str, Any]:
    ...
    if evolver:
        try:
            ...
            return {'status': 'ok', 'target': target, 'total_docs': total_docs, 'total_versions': total_versions}
        except Exception:
