...
def scan_docs(paths: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    if paths is None:
        candidates = [
            'AUTONOMOUS_PROMPTS.md', 'cockpit.html', 'omni_gateway.py', 'rehydrate_master.py',
            'inspect_firestore.py', 'cloudbuild.yaml', 'Dockerfile', 'README.md'
        ]
    else:
        candidates = paths
...
def merge_manifest_with_index(manifest: Dict[str, Any], index: List[Dict[str, Any]]) -> Dict[str, Any]:
    manifest = manifest.copy()
    manifest['doc_index'] = {e['path']: {'sha1': e['sha1'], 'mtime': e['mtime'], 'size': e['size']} for e in index}
...
def write_manifest_firestore(client: 'firestore.Client', manifest: Dict[str, Any], doc_id: str = 'rehydrate_master_manifest') -> bool:
    try:
...
def write_doc_index_firestore(client: 'firestore.Client', index: List[Dict[str, Any]], doc_id: str = 'doc_index') -> bool:
    try:
        payload = {e['path']: {'sha1': e['sha1'], 'mtime': e['mtime'], 'size': e['size']} for e in index}
        client.collection(FIRESTORE_COLLECTION).document(doc_id).set({'index': payload, 'scanned_at': datetime.utcnow().isoformat() + 'Z'})
...
def ensure_calendar_service() -> Optional[Any]:
    ...
    try:
        if creds_path and os.path.exists(creds_path):
            sc = service_account.Credentials.from_service_account_file(creds_path, scopes=['https://www.googleapis.com/auth/calendar'])
            service = build('calendar', 'v3', credentials=sc)
            logger.info('Calendar service initialized using service account')
            return service
        else:
            logger.warning('No service account credentials found; calendar sync skipped')
            return None
    except Exception:
...
def create_calendar_event(service: Any, summary: str, description: str, start_dt: datetime, duration_minutes: int = 60) -> Optional[str]:
    try:
...
def interactive_todo_loop(client: Optional['firestore.Client']):
    ...
    def list_todos():
        if coll:
            ...
            for d in docs:
                data = d.to_dict()
                print(f"- [{ 'x' if data.get('done') else ' ' }] {d.id}: {data.get('title')} (created: {data.get('created')})")
        else:
            print('Firestore not available; no remote TODOs.')

    def add_todo(title: str):
        item = {'title': title, 'done': False, 'created': datetime.utcnow().isoformat() + 'Z'}
        if coll:
            doc = coll.document()
            doc.set(item)
...
def main():
    ...
    p.add_argument('--write-firestore', action='store_true', help='Write manifest and doc index to Firestore')
    p.add_argument('--sync-calendar', action='store_true', help='Create a calendar event for the next action (if credentials present)')
    p.add_argument('--interactive-todos', action='store_true', help='Open interactive todo list (backed by Firestore when available)')
    ...
    if args.scan:
        ...
        if args.sync_calendar:
            ...
            if service:
                ...
                desc = f"Rehydrate run at {merged.get('last_scanned')} - manifest updated"
                start = datetime.utcnow() + timedelta(minutes=5)
                create_calendar_event(service, summary, desc, start)
            else:
                logger.warning('Calendar service not initialized; skipping calendar sync')

        print(json.dumps({'scanned': len(index), 'manifest_version': merged.get('manifest_version')}, indent=2))
