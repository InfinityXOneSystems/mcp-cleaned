...
if not os.environ.get("GOOGLE_APPLICATION_CREDENTIALS"):
    logger.warning(
        "⚠ P1 SECRETS: GOOGLE_APPLICATION_CREDENTIALS not set. Firestore may fail."
    )
...
try:
    from mcp_http_adapter_p1 import DEMO_MODE as MCP_DEMO_MODE
    from mcp_http_adapter_p1 import (
        REGISTRY_HASH,
    )
    from mcp_http_adapter_p1 import router as mcp_router

    app.include_router(mcp_router)
    MCP_AVAILABLE = True
    logger.info(
        f"✓ P1: MCP adapter mounted (hash={REGISTRY_HASH}, demo={MCP_DEMO_MODE})"
    )
...
async def gateway_health():
    ...
    if (
        components["adapter"] == "unavailable"
        or components["firestore"] == "unavailable"
    ):
...
logger.info(
    f"P1: Omni Gateway initialized (demo={DEMO_MODE}, firestore={FIRESTORE_PROJECT})"
)
