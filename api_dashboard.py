...
async def get_dashboard_data(
    system: str = Query(
        "all", description="Filter by system: prediction|real_estate|loan|all"
    )
):
    """
    ...
    for i in range(10):
        timeline.append(
            {
                "ts": (now - timedelta(minutes=10 * (9 - i))).isoformat() + "Z",
        ...
        "prediction": [
            {
                ...
                "agent": "predictor",
                "summary": "Short-term demand increase",
                "confidence": 0.82,
            },
            {
                "created_at": (now - timedelta(minutes=20)).isoformat() + "Z",
                "agent": "validator",
                "summary": "Model drift observed",
                "confidence": 0.68,
            },
        ],
        "real_estate": [
            {
                "created_at": (now - timedelta(minutes=8)).isoformat() + "Z",
                "agent": "ingestor",
                "summary": "New MLS feed ingested",
                "confidence": 0.91,
            },
            {
                "created_at": (now - timedelta(minutes=26)).isoformat() + "Z",
                "agent": "organizer",
                "summary": "Cluster: suburban rental growth",
                "confidence": 0.79,
            },
        ],
        "loan": [
            {
                ...
                "agent": "predictor",
                "summary": "Higher default risk in cohort X",
                "confidence": 0.74,
            },
            {
                "created_at": (now - timedelta(minutes=35)).isoformat() + "Z",
                "agent": "validator",
                "summary": "Verification pending for 3 apps",
                "confidence": 0.66,
            },
        ],
    }
